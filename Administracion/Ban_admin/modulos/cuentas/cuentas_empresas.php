                            ...
                            <div class="box-body">
                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        <?php
                                        $a = new Conexion();
                                        $sql = ("select cl.id,cl.nombre as cliente,cl.apellido,pj.razon_social,c.num_cuenta,c.saldo,c.actived,tc.nombre,c.estado,c.id as id_cuenta from cliente cl, cuenta c, tipo_cuenta tc, persona_juridica pj where
                                                      cl.id=c.cliente_id
                                                      And
                                        ...
                                        while ($rows = pg_fetch_array($resul)) {
                                            ...
                                            <tr>
                                                ...
                                                <td><?php 
                                                    ...
                                                    if($rows['estado']==1){
                                                        ?>
                                                      <span class='label label-warning'> <font class='h5'>Cuenta Principal</font>
                                                        <?php        
                                                    } else{
                                                        ?>
                                                       <span class='label label-info'><font class='h5'>Normal</font>
                                                      <?php
                                                    }
                                                    ?>
                                                </td>
                                                <td>
                                                    <div class="btn-group">
                                                        ...
                                                        <ul class="dropdown-menu" role="menu">
                                                            ...
                                                            if ($total >=2)
                                                            {
                                                                 $est= $a->execute1("select estado from cuenta where (estado='null') and  cliente_id= ".$rows['id']." ");
                                                                while ($ro= pg_fetch_array($est))
                                                                {
                                                                    echo  $estado=$ro['estado'];
                                                                }
                                                                if ($estado==1)
                                                                {
                                                                 ?> 
...
//INSERTO EN LA TABLA CUENTAS
    ...
    */
    function cuenta_principal(){
        ...
        $principal = 1;
        $sql9 = ("update cuenta set estado=$principal where id=$id_cuenta");
        $a->execute($sql9);
   ...
   function actualizar() {
        ...
        if ($valor5 == 'Activo') {
            ...
            $sql1 = ("update cliente set nombre='$nombre', apellido='$apellido' WHERE id=$id_cliente");
            $sql2 = ("update persona_juridica set nit=$nit, direccion='$direccion', correo='$correo',razon_social='$razon' where cliente_id=$id_cliente");
            $a->execute($sql1);
            $a->execute($sql2);
            $a->mensaje_actualizar($modulo, $archivo, $accion);
        } elseif ($valor5 == "Inactivo") {
            $estadoInactivo5 = "false";
            $sql3 = ("update cliente set nombre='$nombre', apellido='$apellido', Actived=$estadoInactivo5 WHERE id=$id_cliente");
            $sql4 = ("update cuenta set Actived=$estadoInactivo5 where cliente_id=$id_cliente");
            $a->execute($sql3);
            $a->execute($sql4);
            $a->mensaje_actualizar($modulo, $archivo, $accion);
        } elseif ($valor6 == "Inactivo") {
            $sql5 = ("update cliente set nombre='$nombre', apellido='$apellido' WHERE id=$id_cliente");
            $sql6 = ("update persona_juridica set nit=$nit, direccion='$direccion', correo='$correo',razon_social='$razon' where cliente_id=$id_cliente");
            $a->execute($sql5);
            $a->execute($sql6);
            $a->mensaje_actualizar($modulo, $archivo, $accion);
        } elseif ($valor6 == "Activo") {
            $estadoInactivo6 = "true";
            $sql7 = ("update cliente set nombre='$nombre', apellido='$apellido' , Actived=$estadoInactivo6 WHERE id=$id_cliente");
            $sql8 = ("update cuenta set Actived=$estadoInactivo6 where cliente_id=$id_cliente");
            $a->execute($sql7);
            $a->execute($sql8);
            $a->mensaje_actualizar($modulo, $archivo, $accion);
        }
        if ($valor7 == 'Activo') {
            ...
            $sql9 = ("update cuenta set Actived=$estadoInactivo7 where id=$id_cuenta");
            $a->execute($sql9);
            $a->mensaje_actualizar($modulo, $archivo, $accion);
